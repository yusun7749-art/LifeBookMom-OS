"""Verify that a LifeBookMom CMS record exists as the expected private Blogger draft."""
...
ELIGIBLE_STATUS = "BLOGGER_PREVIEW_DRAFT_CREATED"
...
def validate_cms_record(draft: dict[str, Any]) -> None:
    if draft.get("status") != ELIGIBLE_STATUS:
        raise ValueError(f"{ELIGIBLE_STATUS} 상태만 검증할 수 있습니다.")
    for field in ("content_id", "title", "html", "blogger_blog_id", "blogger_post_id"):
...
def compare_draft(cms: dict[str, Any], remote: dict[str, Any]) -> list[dict[str, str]]:
    ...
    remote_id = str(remote.get("id", "")).strip()
    if remote_id != str(cms["blogger_post_id"]):
        issues.append({"code": "POST_ID_MISMATCH", "message": f"CMS={cms['blogger_post_id']} Blogger={remote_id}"})
    if str(remote.get("title", "")).strip() != str(cms["title"]).strip():
        issues.append({"code": "TITLE_MISMATCH", "message": "Blogger 제목이 CMS 제목과 다릅니다."})
    if _normalize_html(remote.get("content")) != _normalize_html(cms["html"]):
...
def run(
    ...
    *,
    ...
    remote = service.posts().get(
        blogId=str(cms["blogger_blog_id"]),
        postId=str(cms["blogger_post_id"]),
        view="ADMIN",
    ).execute()
    ...
    report = {
        ...
        "status": "BLOGGER_DRAFT_VERIFICATION_PASS" if passed else "BLOGGER_DRAFT_VERIFICATION_FAIL",
        "passed": passed,
    ...
    updated = dict(cms)
    updated.update({
        "status": "BLOGGER_DRAFT_VERIFIED" if passed else "BLOGGER_DRAFT_VERIFICATION_FAILED",
        "next_stage": "EDITOR_REVIEW" if passed else "BLOGGER_DRAFT_REWORK",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="생활백서맘 Blogger 비공개 초안 실제 검증")
    ...
    try:
        ...
        print(json.dumps({"status": "BLOGGER_DRAFT_VERIFICATION_ERROR", "error": str(exc)}, ensure_ascii=False))
        return 2
