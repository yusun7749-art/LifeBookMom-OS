    ...
    <section className="rounded-[2rem] border border-[#E4D5BE] bg-white p-7">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-2 text-[#7A6B5B]">모든 센터가 하나의 기준 데이터로 연결됩니다.</p>
        </div>
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-5">
        {[
          ["Version", osCore.currentVersion],
          ["Branch", osCore.branch],
          ["Project", osCore.currentProject],
          ["Baseline", osCore.baselineCommit],
          ["Recovery", osCore.recoveryStatus],
        ].map(([label, value]) => (
          <div key={label} className="rounded-2xl bg-[#FFFDF8] p-4">
            <p className="text-xs font-black text-[#9A8B7B]">{label}</p>
          ...
          <div key={group.key} className="rounded-3xl bg-[#F7F1E8] p-5">
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
              {osModules
                ...
                .map((module) => (
                  <Link key={module.key} href={module.href} className="rounded-2xl bg-white p-4 transition hover:-translate-y-1 hover:shadow-md">
      ...
      <div className="mt-6 space-y-3">
        {osTimeline.map((item) => (
          <div key={item.project} className="rounded-2xl bg-[#FFFDF8] p-4">
            ...
            <p className="mt-2 text-sm font-bold text-[#7A6B5B]">{item.version} · {item.commit} · {item.reason}</p>
          </div>
        ))}
      </div>
      ...
      <div className="mt-6 grid gap-3 md:grid-cols-5">
        {osRules.map((rule) => (
          <div key={rule} className="rounded-2xl bg-[#EFF8F2] p-3 text-sm font-bold text-[#2F6B4F]">{rule}</div>
        ))}
      </div>
    </section>
  );
